...
const MainPage = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      const data = await fetch(`${baseUrl}/test/all`, {
        method: 'GET',
    ...
    const checkData = async () => {
      ...
      !!keys && typesData.filter(item => !keys.find(key => key === item)).length === 0 ? undefined : await getData();
  ...
  const sendUpdateRequest = async () => {
    if (user) {
      ...
      if (result.status === 200) {
        ...
        Alert.alert('Успех!', 'Через несколько секунд нажжмите на "Получить обновления".', [
          {
            text: 'OK',
          },
        ]);
  ...
  const checkUpdateRequest = async () => {
    const result = await fetch(`${baseUrl}/messages?companyId=${user.companies[0]}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    }).then(res => res.json());
  ...
  const _logoutAsync = async () => {
    Alert.alert('Выйти из пользователя?', '', [
      {
        text: 'Подтвердить',
        onPress: async () => {
          ...
          return Alert.alert('Ошибка сервера', res.result, [
            {
              text: 'OK',
            },
          ]);
  ...
  return (
    <View style={styles.container}>
      <View style={{ flex: 2 }}>
        <TouchableHighlight style={styles.viewButton} onPress={() => navigate('DirectoryPage')}>
          <Text style={styles.viewButtonText}>Справочник</Text>
        </TouchableHighlight>
        <View style={{ flex: 1 }}>
          <TouchableOpacity style={styles.createButton} onPress={() => navigate('DocumentPage')}>
            <Text style={styles.createButtonText}>Документы</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.createButton}
            onPress={async () => {
              !resUpd ? sendUpdateRequest() : checkUpdateRequest();
            }}
          >
      ...
      <View style={{ flexDirection: 'row-reverse' }}>
        ...
        <TouchableOpacity onPress={_logoutAsync} style={styles.systemButtons}>
          <MaterialCommunityIcons size={30} color="#FFF" name="account-convert" />
        </TouchableOpacity>
      </View>
