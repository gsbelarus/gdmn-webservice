...
const newMessage = async (ctx: any) => {
  if (ctx.isAuthenticated()) {
    ...
    if (!ctx.state.user.companies) {
      ...
      logger.warn(
        `The User (${ctx.state.user.id}) does not belongs to the Company (${companyId})`
      );
      return;
    }

    if (
      !(ctx.state.user.companies as Array<string>).find(
        item => item === companyId
      )
    ) {
      ctx.body = JSON.stringify({
      ...
      logger.warn(
        `The User (${ctx.state.user.id}) does not belong to the Company (${companyId})`
      );
      return;
    }
...
const getMessage = async (ctx: any) => {
  ...
  try {
    const nameFiles = await promises.readdir(
      `${PATH_LOCAL_DB_MESSAGES}${companyId}`
    );
    for (const newFile of nameFiles) {
      const data = await readFile(
        `${PATH_LOCAL_DB_MESSAGES}${companyId}\\${newFile}`
      );
    ...
    ctx.body = JSON.stringify({
      ...
      result: result.filter(
        res => res.head.consumer === ctx.state.user.userName
      )
    });
    logger.info("get message");
  } catch (e) {
    logger.trace(
      `Error reading data from directory ${PATH_LOCAL_DB_MESSAGES}${companyId} - ${e}`
    );
    console.log(
      `Error reading data from directory ${PATH_LOCAL_DB_MESSAGES}${companyId} - ${e}`
    );
...
const remove = async (company: string, uid: string) =>
  await removeFile(`${PATH_LOCAL_DB_MESSAGES}${company}\\${uid}.json`);
